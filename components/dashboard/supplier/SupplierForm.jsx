...
import {addSupplier, updateSupplier} from "@/lib/querySuppliers";
import {ProgressLoader} from "@/components";
import {showErrorToast, showSuccessToast} from "@/utils/toastUtil";

export default function SupplierForm({supplier = null, newSupplierCode = ''}) {
    ...
    const [formData, setFormData] = useState({
        name: supplier?.name || '',
        code: supplier?.code || newSupplierCode || '',
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            ...
            showSuccessToast(`Supplier ${updatedSupplier?.name} updated successfully`);
            router.push(`/dashboard/suppliers`);
        } catch (error) {
            console.error('Error updating supplier: ', error);
            showErrorToast('An error occurred while updating the supplier');
        } finally {
            setLoading(false);
        }
    }
    ...
    const handleCreateSubmit = async () => {
        ...
        try {
            const addedSupplier = await addSupplier(formData);
            showSuccessToast(`Supplier ${addedSupplier?.name} created successfully`);
            router.push(`/dashboard/suppliers`);
        } catch (error) {
            console.error('Error adding supplier:', error);
            showErrorToast('An error occurred while creating the supplier');
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <div className={`max-w-4xl mx-auto py-8`}>
            <form onSubmit={handleSubmit} className={`space-y-8`}>
                <BasicInformationForm formData={formData} handleChange={handleChange} errors={errors}/>
                <ContactInformationForm formData={formData} handleChange={handleChange} errors={errors}/>
                <BusinessInformationForm formData={formData} handleChange={handleChange} errors={errors}/>
                <NotesForm formData={formData} handleChange={handleChange}/>

                {/* Form Actions */}
                <div className={`flex items-center justify-end gap-4 pt-6 border-t border-gray-700`}>
                    <button type={`button`} onClick={router.back} disabled={loading} className={`dashboard-cancel-btn`}>
                        <HiOutlineXMark className={`size-5`}/>
                        Cancel
                    </button>
                    <button type={`submit`} disabled={loading} className={`dashboard-submit-btn`}>
                        {loading ? (
                            <>
                                <ProgressLoader/>
...
function BasicInformationForm({formData, handleChange, errors = {}}) {
    return (
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-6`}>
                ...
                <div className={`md:col-span-2`}>
                    <div className={`flex items-center gap-3`}>
                        ...
                        />
                        <label htmlFor={`is_active`} className={`dashboard-form-label`}>Active supplier</label>
                    </div>
                </div>
            </div>
        </div>
...
function ContactInformationForm({formData, handleChange, errors = {}}) {
    return (
        <div className={`bg-gray-800 border border-gray-700 rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold mb-6 flex items-center gap-2`}>
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-6`}>
                <div>
                    <label htmlFor={`contact_person`} className={`dashboard-form-label mb-2`}>Contact Person</label>
                    <input
                        type={`text`}
                        id={`contact_person`}
                        name={`contact_person`}
...
function BusinessInformationForm({formData, handleChange, errors = {}}) {
    return (
        <div className={`bg-gray-800 border border-gray-700 rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold mb-6 flex items-center gap-2`}>
...
function NotesForm({formData, handleChange}) {
    return (
        <div className={`bg-gray-800 border border-gray-700 rounded-lg p-6`}>
            <h2 className={`text-xl font-semibold mb-6 flex items-center gap-2`}>
                <HiOutlineDocumentText className={`size-6 text-teal-400`}/>
