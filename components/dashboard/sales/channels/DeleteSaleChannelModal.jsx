...
export default function DeleteSaleChannelModal({deleteModal, setDeleteModal, handleDelete, isLoading = false}) {
    return (
        <div className={`fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50`}>
            <div className={`bg-gray-800 rounded-lg max-w-md w-full p-6`}>
                <h3 className={`text-lg font-medium mb-4`}>Delete Sale Channel</h3>
                <p className={`text-gray-400 mb-6`}>
                    Are you sure you want to delete "{deleteModal.channel?.name}"? This action cannot be undone.
                </p>
                <div className={`flex justify-end space-x-3`}>
                    <button
                        onClick={() => setDeleteModal({open: false, channel: null})}
                        disabled={isLoading}
