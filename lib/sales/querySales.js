...
export async function getSalesForCurrentOrganization(options = {}) {
    ...
    const {searchTerm = '', statusFilter = 'all', page = 1, limit = 10} = options;

    let query = supabase
        .from('sales')
        .select('*, customer: customers(id, first_name, last_name, business_name, type)', {count: 'exact'});
    ...
    const sales = data.map(sale => ({
        ...
        customer: {
            ...sale.customer,
            name: getCustomerDisplayName(sale.customer)
        }
    }));
...
export async function getPaymentMethodsForSale() {
    const supabase = await createClient();
